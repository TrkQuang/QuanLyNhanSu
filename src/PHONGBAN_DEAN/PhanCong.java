...
public class PhanCong {
    ...
    private Date parseDate(String dateStr){
        // Nếu dateStr null hoặc rỗng, trả về null
    ...
    private long tinhSoNgayLech() {
        ...
        // Nếu một trong hai ngày null thì không tính được
        if (ngayHoanThanhDate == null || deadlineDate == null) {
            return 0;
        }
    ...
    public String getDanhGia() {
        // Nếu thiếu dữ liệu ngày thì không có đánh giá
        if (isNullOrEmpty(ngayHoanThanh) || isNullOrEmpty(deadLineDeAn)) {
    ...
    public void nhapPhanCong(){
        ...
        while(!DataCenter.dsda.tonTaiDeAn(this.maDA)) {
        ...
        // Lấy thông tin đề án để điền ngày mặc định
        DeAn deAn = DataCenter.dsda.timDeAn(this.maDA);
        
        System.out.print("Ngày phân công (yyyy-MM-dd) (enter để dùng ngày BD của đề án): ");
        this.ngayPhanCong = sc.nextLine().trim();
        if (this.ngayPhanCong.isEmpty() && deAn != null) {
            this.ngayPhanCong = deAn.getNgayBD();
            System.out.println("  → Dùng ngày BD đề án: " + this.ngayPhanCong);
        }
        
        System.out.print("Deadline đề án (yyyy-MM-dd) (enter để dùng ngày KT của đề án): ");
        this.deadLineDeAn = sc.nextLine().trim();
        if (this.deadLineDeAn.isEmpty() && deAn != null) {
            this.deadLineDeAn = deAn.getNgayKT();
            System.out.println("  → Dùng deadline đề án: " + this.deadLineDeAn);
        }
